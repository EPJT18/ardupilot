...
void AP_Airspeed::update_error_estimate(uint8_t i)
{
    ...
    float pressure_max =   fabsf(state[i].filtered_pressure) + param[i].pressure_error;
    if (pressure_max<0){
        pressure_max = 0.1;
    }
    float pressure_min =   fabsf(state[i].filtered_pressure) - param[i].pressure_error;
    if (pressure_min<0){
        pressure_min = 0.1;
    }
    
    state[i].error_pos = fabsf( sqrtf(pressure_max * param[i].ratio) -state[i].airspeed);
    state[i].error_neg = fabsf(state[i].airspeed - sqrtf(pressure_min * param[i].ratio));
...
void AP_Airspeed::check_sensor_values_consistent(uint8_t i)
{
    ...
    for (uint8_t j=0; j<AIRSPEED_MAX_SENSORS; j++) {
        if(j != i){
            //if the other sensor is enabled and healthy
            if((state[j].failures.param_use_backup > 0 ||param[j].use >0) && state[j].healthy)
            {
