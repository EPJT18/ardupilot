...
bool Plane::verify_nav_wp(const AP_Mission::Mission_Command& cmd)
{
    ...
    // are there really short lengths involved?
    if(cmd.content.location.get_distance_NE(mission.get_next_location(cmd.content.location)).length()<1.0f || plane.current_loc.get_distance_NE(cmd.content.location).length()<1.0f  ){
    ...
    if (cmd_acceptance_distance > 0) {
        ...
        acceptance_distance_m = plane.current_loc.get_distance_NE(cmd.content.location).normalized()*cmd_acceptance_distance;
    }
    else if(next_nav_cmd.id == MAV_CMD_NAV_VTOL_LAND) {
        acceptance_distance_m = plane.current_loc.get_distance_NE(cmd.content.location).normalized();
    }
    ...
    if(acceptance_distance_m.length()<0.05 ){
        return false;
    }
    ...
    const bool doesIntersect = Vector2f::segment_intersection(lineToIntersect,-lineToIntersect.normalized(),PlaneRelToWaypoint,line2,returnValue);
